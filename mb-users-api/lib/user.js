...
User.prototype.get = function(req, res) {
  ...
  var query = {};
  // Allow queries for mixed case email addresses
  if (req.query.email && req.query.exactCase == 1) {
    ...
    function(err, doc) {
      ...
      for (var i = 0; i < keysLength; i++) {
        if (allowedKeys.indexOf(keys[i]) === -1) {
          dslogger.error('Unsupported key ' + keys[i] + ' from ' + req.protocol + '://' + req.get('host') + req.path + ' POSTed: ' + self.request.body[keys[i]]);
          continue;
        }
        if (self.request.body[keys[i]] && keys[i] !== "campaigns" && keys[i] !== "subscriptions") {
          ...
          if (keys[i] == "drupal_register_timestamp") {
            ...
            updateArgs['$set'].drupal_uid = self.request.body[keys[i]];
          }
          else if (keys[i] == "birthdate") {
            updateArgs['$set'].birthdate = self.request.body[keys[i]];
          }
            ...
            updateArgs['$set'].language = self.request.body[keys[i]]
          }
          else if (keys[i] == "email") {
            updateArgs['$set'].email = self.request.body[keys[i]].toLowerCase()
          }
          else if (keys[i] == "user_country") {
            updateArgs['$set'].country_code = self.request.body[keys[i]]
          }
          else {
      ...
      // Add created Date stamp to all new documents
      if (doc == null)  {
        updateArgs['$set'].created = new Date();
      }
...
User.prototype.delete = function(request, response) {
  ...
  console.log("prototype.delete self.email: " + self.email);

  this.docModel.remove(
    {email: self.email},
    function (err, num) {
      if (err) {
        console.log('ERROR prototype.delete: ' + err);
        self.response.send(500, err);
        dslogger.error(err);
      }

      if (num == 0) {
        console.log('No documents found to delete for email: ' + self.email);
        self.response.send(200, 'No documents found to delete for email: ' + self.email);
        dslogger.error('No documents found to delete for email: ' + self.email);
      }
      else {
        console.log('Deleted ' + num + ' document(s) for email: ' + self.email);
        self.response.writeHead(200, {'Content-Type': 'application/json'});
        var resMessage = 'Deleted ' + num + ' document(s) for email: ' + self.email;
        self.response.write(JSON.stringify({ message: resMessage }));
        // self.response.send(200, 'Deleted ' + num + ' document(s) for email: ' + self.email);
